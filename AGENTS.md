...
- **Mobile shell:** the app owns a `100dvh` flex viewport below `md`; only its content pane scrolls, while `BottomNav` remains a normal-flow non-scrolling sibling. Do not restore viewport-fixed mobile navigation; page-height changes make it unstable in iOS standalone mode (D44).
